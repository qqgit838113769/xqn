(function($) {
	var defaultSettings = {
		...
		maximumFilesUpload : 1,//最大文件上传数(当此属性大于1时，buttonFeature属性只能为true)  
		...
		perviewImgStyle : { width: '400px', height: '400px', border: '1px solid #C0C0C0' }
	//用于设置图片预览时的样式（可不设置，在不设置的情况下多文件上传时只能显示一张图片），如{ width: '100px', height: '100px', border: '1px solid #ebebeb' }  
	};
	...
	$.fn.uploadFile = function(settings) {
		...
		if (settings.perviewImageElementId) {
			...
			if (!settings.perviewImgStyle) {
				var perviewImg = document
						.getElementById(settings.perviewImageElementId);
		...
		return this.each(function() {
			...
			window.setTimeout(function() {
				...
				$(upload.getIframeContentDocument().body.lastChild).on("load",
						function() {
...
UploadAssist.prototype = {
	...
	createInputFile : function() {
		...
		input.onchange = function() {
			...
			if (!that.checkFileIsUpload(fileSuf, that.settings.fileSuffixs)) {
				that.settings.onCheckUpload(that.settings.errorText.replace(
						"{0}", fileSuf));
				return;
			}
			...
			if (that.settings.perviewImageElementId) {
				if (!that.settings.perviewImgStyle) {
					perviewImage.beginPerview(this,
							that.settings.perviewImageElementId);
				} else {
					var ul = perviewImage
							.getPerviewRegion(that.settings.perviewImageElementId);
	...
	getUploadInput : function() {
		var inputs = this.getIframeContentDocument().getElementsByTagName(
				"input");
	...
	chooseFile : function() {
		...
		if (this.choseFilePath.length == this.settings.maximumFilesUpload) {
			if (this.settings.maximumFilesUpload < 1) {
				this.choseFilePath.length = 0;
				var files = this.getIframeContentDocument()
...
var perviewImage = {
	...
	//开始图片预览  
	beginPerview : function(/*文件上传控件实例*/file, /*需要显示的元素id或元素实例*/
			perviewElemId, dcmt) {
...
//			var idName = fileName.substring(0,fileName.indexOf("."));
			//var idName = getUUID();
			var img = document.createElement("img");
			temp.id = idName;
			temp.appendChild(img);
			preview_div.appendChild(temp);
			
			img.name = "images";
			img.style.visibility = "hidden";
			img.onload = function() {
				var rect = perviewImage.clacImgZoomParam(MAXWIDTH, MAXHEIGHT,
						img.offsetWidth, img.offsetHeight);
				img.style.width = Math.round(rect.width * 0.98) + 'px';
				img.style.height = Math.round(rect.height * 0.98) + 'px';
				//img.style.width = "100%";
	...
	createPreviewElement : function(/*上传时的文件名*/file, /*预览时的样式*/style) {
		...
		close.style.top = -410 + "px";
		close.style.left = 590 + "px";
